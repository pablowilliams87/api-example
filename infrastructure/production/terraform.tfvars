region            = "us-east-1"
environment       = "production"
vpc_subnet        = "10.30.0.0/16"
url_measurements  = "https://gist.githubusercontent.com/jvillarf/040c91397d779d4da02fff54708ca935/raw/f1dbbcbfbc4e3daace7d907a3cc5b716ef808014/environment_airq_measurand.csv"
public_subnets    = { us-east-1a = "10.30.10.0/24", us-east-1b = "10.30.11.0/24" }
private_subnets   = { us-east-1a = "10.30.15.0/24", us-east-1b = "10.30.16.0/24" }
ecs_ami_id        = "ami-0669eafef622afea1"
ecs_cluster       = "production-ecs-measurement-app"
ecs_instance_type = "t3.medium"
ecs_public_key    = "ssh-rsa  pablow@DESKTOP-K7SL4L"
asg_capacity      = [4,4,8]
rds_db            = "environment_airq_measurand"
rds_user          = "postgres"
rds_instancetype  = "db.t3.micro"
image_tag         = "pablowilliams87/measurement-app:1.2"
container_name    = "production-measurement-app"
container_count   = 4
container_cpu     = 1024
container_memory  = 1024
flask_env         = "production"
init_db           = "0"